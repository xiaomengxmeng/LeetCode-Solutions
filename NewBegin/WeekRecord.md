...
# 📊 5天密集周复习总结 (2026-06-01 ~ 2026-06-05)
...
- **28 道题全部独立写出，零失误** ✅ — 说明前六周的学习内容已经深度内化
- **无需重做任何题目** — 没有薄弱项需要巩固
- **复习验证了学习方法有效**：模板化学习 + 及时记录 + 间隔重复，确实能让知识沉淀

---

**5天密集周复习圆满完成！准备进入动态规划阶段！** 🚀

---

# 📊 第七周学习总结 (2026-06-08 ~ 2026-06-12)
...
| 日期 | 完成题目 | 核心技巧 |
|:----:|:---------|:---------|
| 06-08 | Climbing Stairs, Min Cost Climbing Stairs, House Robber | 斐波那契型 DP、最值型 DP、选或不选型 DP |
| 06-09 | Unique Paths, Unique Paths II, Perfect Squares | 二维路径 DP、障碍物处理、最值型 DP |
| 06-10 | Coin Change, Longest Increasing Subsequence | 最值型 DP 进阶、子序列 DP |
| 06-11 | Longest Common Subsequence, LPS, Longest Palindromic Substring | 双序列 DP、LCS 应用、中心扩散 |
| 06-12 | Word Break, Best Time to Buy and Sell Stock I/II | 布尔型 DP、状态追踪、贪心/状态机 |
...
| 类型 | 状态定义 | 转移方程 | 初始条件 |
|:----|:---------|:---------|:---------|
| 70 爬楼梯 | dp[i] = 爬到第i阶的方法数 | dp[i] = dp[i-1] + dp[i-2] | dp[1]=1, dp[2]=2 |
| 746 最小花费 | dp[i] = 爬到第i阶的最小花费 | dp[i] = min(dp[i-1]+cost[i-1], dp[i-2]+cost[i-2]) | dp[0]=0, dp[1]=0 |
| 198 打家劫舍 | dp[i] = 前i+1家的最高金额 | dp[i] = max(dp[i-2]+nums[i], dp[i-1]) | dp[0]=nums[0], dp[1]=max(nums[0],nums[1]) |

**递推三连演变**：
```
70（求和 +） → 746（最值 min） → 198（最值 max + 可跳过）
```
...
**核心思想**：dp[i][j] 表示网格位置的状态

| 题号 | 状态定义 | 转移方程 | 边界处理 |
|:---:|:---------|:---------|:---------|
| 62 | dp[i][j] = 路径数 | dp[i][j] = dp[i-1][j] + dp[i][j-1] | 第一行/列初始化为 1 |
| 63 | dp[i][j] = 路径数 | 障碍物位置 = 0，否则同62 | break 截断边界 |

**障碍物处理口诀**："遇障碍 → dp=0，边界遇障碍 → break截断"

---

### 3️⃣ 最值型 DP 统一框架 (279/322)

**核心思想**：dp[i] = min/max over 所有选择 (dp[i - 选择] + 代价)

| 题号 | 选择空间 | 转移方程 | 不可达处理 |
|:---:|:---------|:---------|:---------|
| 279 | j² ≤ i 的所有平方数 | dp[i] = min(dp[i-j²] + 1) | dp[0]=0 |
| 322 | 所有硬币面额 | dp[i] = min(dp[i-coin] + 1) | INT_MAX/2 占位 |

**统一框架**：
```cpp
dp[i] = 最坏情况;  // i 或 INT_MAX/2
for (选择 in 所有可能) {
    if (选择有效 && dp[i-选择] 可达)
        dp[i] = min(dp[i], dp[i-选择] + 1);
}
```
...
**核心思想**："以 nums[i] 结尾"的状态定义

| 题号 | 状态定义 | 转移方程 | 答案位置 |
|:---:|:---------|:---------|:---------|
| 300 LIS | dp[i] = 以nums[i]结尾的LIS长度 | dp[i] = max(dp[j]+1) for j<i & nums[j]<nums[i] | max(dp[i]) |

**关键点**：
- dp 全初始化为 1（每个元素自身至少长度1）
- 答案不是 dp[n-1]，而是全局 max
- O(n log n) 优化：贪心 + 二分（tails数组）

---

### 5️⃣ 双序列 DP (1143)

**核心思想**：dp[i][j] 表示两个序列前缀的状态

| 题号 | 状态定义 | 转移方程 | 关键技巧 |
|:---:|:---------|:---------|:---------|
| 1143 LCS | dp[i][j] = LCS长度 | 相等: dp[i-1][j-1]+1; 不等: max(dp[i-1][j], dp[i][j-1]) | size+1技巧 |
| 516 LPS | LCS(s, reverse(s)) | 复用1143代码 | 问题转化 |

**size+1技巧**："dp(m+1)×(n+1) 让边界自动为0，下标1-based，简化代码"

---

### 6️⃣ 字符串分割 DP (139)

**核心思想**：布尔型 dp，判断前缀能否被拆分

| 题号 | 状态定义 | 转移方程 | 剪枝技巧 |
|:---:|:---------|:---------|:---------|
| 139 Word Break | dp[i] = 前i字符能否拆分 | dp[i] = ∃j: dp[j] && s[j..i)∈字典 | 子串长度 > maxSize 跳过 |

**布尔型 DP 特点**："找到即 break，无需遍历所有分割点"
...
| 题号 | 方法 | 核心思想 |
|:---:|:----|:---------|
| 121 | 状态追踪 | minPrice + maxProfit，一次遍历 |
| 122 | 贪心 | 累计所有正价差 Σmax(0, prices[i]-prices[i-1]) |

**状态机 DP 预埋**："hold = max(hold, notHold-price) / notHold = max(notHold, hold+price)"
...
| 类型 | 常见错误 | 正确做法 |
|:----|:---------|:---------|
| dp数组大小 | dp[n] | dp[n+1]（前i个字符/爬到第i阶） |
| 循环边界 | `i < n` | `i <= n`（包含终点） |
| 最值初始化 | dp[i]=0 | dp[i]=i 或 INT_MAX/2（最坏情况） |
| LIS答案 | return dp[n-1] | return max(dp[i])（全局最大） |
| LCS下标 | text1[i] | text1[i-1]（size+1技巧） |
| 不可达标记 | INT_MAX | INT_MAX/2（避免+1溢出） |
| 121初始化 | minPrice=0 | minPrice=INT_MAX |
| 122累加 | sum = max(...) | sum += max(...) |
...
- [x] **全勤 5 天**：连续学习，无断档 ✅
- [x] **DP 入门体系完整**：从斐波那契型到双序列型，7 种 DP 模板全覆盖
- [x] **Medium 题量新高**：10 道 Medium，单周最高 🏆
- [x] **总题数突破 84**：从 78 → 84 道，一周增长 6 道
- [x] **DP 三要素掌握**：状态定义、转移方程、初始条件已成肌肉记忆
- [x] **最值型 DP 统一框架**：279/322/300 共享同一模板

---

## 📊 七周对比

| 维度 | W1 数组 | W2 链表 | W3 栈 | W4 二叉树 | W5 图 | W6 二分+回溯 | **W7 DP** |
|:----:|:-------:|:-------:|:-----:|:---------:|:----:|:-----------:|:---------:|
| 题目数 | 16 | 11 | 6 | 20 | 12 | 13 | **14** |
| Easy | 10 | 7 | 2 | 10 | 1 | 0 | **4** |
| Medium | 6 | 4 | 3 | 10 | 11 | 13 | **10** |
| Hard | 0 | 0 | 1 | 0 | 0 | 0 | **0** |
| 核心技巧 | 8种 | 5种 | 4种 | 10种 | 5种 | 二分+回溯 | **7种DP模板** |
| 重点 | 哈希/窗口 | 链表 | 单调栈 | 二叉树 | 图 | 核心算法 | **动态规划** |

---

## 💡 学习心得

1. **DP 三要素是核心**：状态定义（dp[i]表示什么）、转移方程（从哪来）、初始条件（base case）——每道题都是这三者的组合变化
2. **递推三连的演变路径**：70（+）→ 746（min）→ 198（max+跳过）展示了 DP 从求和到求最值的完整进化
3. **最值型 DP 统一框架**：279/322/300 共享同一模板——"减一个值看之前的状态，取最值"
4. **size+1技巧简化边界**：LCS 用 dp(m+1)×(n+1) 让边界自动为0，避免额外 if 判断
5. **"以 i 结尾"vs"前 i 个"**：LIS 是前者（答案取全局max），Word Break 是后者（答案在dp[n])
6. **问题转化思维**：LPS(s) = LCS(s, reverse(s))——将新问题映射为已解决问题
...
| 主题 | 重点题目 | 核心技能 |
|:----|:---------|:---------|
| DP 进阶 | 0-1背包、完全背包、区间DP | 背包DP模板、区间DP模板 |
| 复习巩固 | 70/746/198/62/63 再练 | 吃透DP基础 |

...
| 日期 | 完成题目 | 核心技巧 |
|:----:|:---------|:---------|
| 06-15 | Partition Equal Subset Sum (416) | 0-1 背包 bool 版 — 能否凑出 target |
| 06-16 | Target Sum (494) | 0-1 背包计数版 — 多少种方式凑出 target |

---

## 🎯 本周核心：0-1 背包 DFS 双模板 ⭐

### 1️⃣ 布尔型 DP — 416. Partition Equal Subset Sum

**核心思想**：`dp[i][j]` = 从前 i 个元素中选，能否恰好凑出和 j

```
dp[i][j] = dp[i-1][j]       // 不选
        || dp[i-1][j-num]   // 选（j >= num）
```
...
### 2️⃣ 计数型 DP — 494. Target Sum

**核心思想**：数学化简 `P = (target + sum) / 2` → 找和为 P 的子集个数
...
| 转移 | `\|\|`（或） | `+=`（累加） |
| 空间 | 二维可优化一维 | 直接一维倒序 |
| 边界 | sum 奇数 → false | 奇偶不匹配 / abs(target)>sum → 0 |
...
| 类型 | 常见错误 | 正确做法 |
|:----|:---------|:---------|
| dp 大小 | 一维 dp 用 `n+1` | 一维 dp 用 `P+1`（P = target/容量） |
| 边界检查 | 漏掉 `abs(target) > sum` | P 可能为负数 → 数组越界 |
| 奇偶判断 | 检查 `sum % 2` | 检查 `(target + sum) % 2` |
| 一维倒序 | 正序遍历 | 倒序保证每个数只用一次 |

---

## 🏆 里程碑

- [x] **0-1 背包 bool 版模板掌握**：416
- [x] **0-1 背包计数版模板掌握**：494 → 数学化简 + 一维倒序
- [x] **总题数达到 86 道** 🎉
...
| 维度 | W1 数组 | W2 链表 | W3 栈 | W4 二叉树 | W5 图 | W6 二分+回溯 | W7 DP | **W8 背包** |
|:----:|:-------:|:-------:|:-----:|:---------:|:----:|:-----------:|:-----:|:----------:|
| 题目数 | 16 | 11 | 6 | 20 | 12 | 13 | 14 | **2** |
| Easy | 10 | 7 | 2 | 10 | 1 | 0 | 4 | **0** |
| Medium | 6 | 4 | 3 | 10 | 11 | 13 | 10 | **2** |
| Hard | 0 | 0 | 1 | 0 | 0 | 0 | 0 | **0** |
| 核心技巧 | 8种 | 5种 | 4种 | 10种 | 5种 | 二分+回溯 | 7种DP模板 | **0-1背包模板** |
| 重点 | 哈希/窗口 | 链表 | 单调栈 | 二叉树 | 图 | 核心算法 | 动态规划 | **背包入门** |

---

## 💡 学习心得

1. **0-1 背包两模板**：bool 版（416）和计数版（494）共享同一个"选/不选"框架，只是 dp 类型和转移操作不同
2. **数学化简是钥匙**：494 从符号分配 → 子集和，没有这一步就解不出来
3. **假期周不勉强**：忙游戏开发就专注做好一件事，学习保持手感就好 🎮
4. **背包进阶铺垫**：完全背包（物品无限次使用）是 0-1 背包的下一步，区别只在遍历方向
...
| 主题 | 重点题目 | 核心技能 |
|:----|:---------|:---------|
| 完全背包 | 322 Coin Change（进阶）、518 Coin Change II | 正序遍历、无限选取 |
| 背包综合 | 474 Ones and Zeroes | 二维 0-1 背包 |
| 背包变体 | 1049 Last Stone Weight II | 背包思维转化 |

---

**第八周圆满完成！0-1 背包两大模板已掌握！** 🎉

...
1. **优先使用用户指定的日期**：如果用户说"今天是29号"，则使用用户指定的日期
2. **其次使用环境日期**：查看 `<env>` 中的 `Today's date`
3. **格式**：`YYYY-MM-DD`（如 `2026-04-29`）
...
#### 场景1：开始新的一天
```
用户："新的一天" 或 "帮我开始今天的算法学习"
→ AI响应流程：
  1. 读取 ProblemRecord.md 检查已完成题目
  2. 回顾昨日进度（读取 DailyRecord/昨日.md）
  3. 推荐1道适合当前水平的题目（Easy 优先）
  4. ⭐ 创建当日文件并生成题目模板（如果不存在）：
     - Mythouth/YYYY-MM-DD.md：包含题目思考模板
     - DailyRecord/YYYY-MM-DD.md：包含题目记录模板（待填写）
...
#### 场景2：解决具体问题 / 下一题
```
用户："我想做 Group Anagrams 这道题" 或 "下一题" 或 "继续"
→ AI响应流程：
  1. ⭐⭐⭐ 【必须第一步】生成题目模板到当日文件：
     - 追加到 Mythouth/YYYY-MM-DD.md（思考模板）
     - 追加到 DailyRecord/YYYY-MM-DD.md（记录模板，待填写）
  2. 展示题目描述和示例
...
→ AI响应流程：
  ...
  3. 整理核心技巧体系
  4. 总结易错点和学习心得
  5. ⭐ 追加/更新 WeekRecord.md（见下方"周总结格式"）
...
>
> 1. **确定日期**：使用用户指定的日期或环境日期
> 2. **生成模板**：在 `Mythouth/YYYY-MM-DD.md` 和 `DailyRecord/YYYY-MM-DD.md` 中追加题目模板
> 3. **然后才能**：展示题目、提出问题
...
### 📝 DailyRecord/YYYY-MM-DD.md 模板（追加）
```markdown
## [难度] 题目名称 (题号)
...
2. 检查当日文件是否存在
   ...
   - 存在 → 准备追加
    ↓
3. ⭐ 追加题目模板到两个文件
   - Mythouth/YYYY-MM-DD.md（思考模板）
   - DailyRecord/YYYY-MM-DD.md（记录模板，待填写）
    ↓
4. 展示题目描述和示例
    ↓
5. 进入 Step 1（问题分析）
...
```
□ 日期是否正确？（用户指定 > 环境日期）
□ 是否已生成模板到 Mythouth/YYYY-MM-DD.md？
□ 是否已生成模板到 DailyRecord/YYYY-MM-DD.md？
□ 模板是否追加（不是覆盖）？
□ 然后才展示题目？
```

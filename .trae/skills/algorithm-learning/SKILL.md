...
→ AI响应流程：
  1. ⭐ 自动创建当日文件（如果不存在）：
     - 检查 Mythouth/YYYY-MM-DD.md 是否存在，不存在则创建空文件
     - 检查 DailyRecord/YYYY-MM-DD.md 是否存在，不存在则创建空文件
  2. 读取 ProblemRecord.md 检查已完成题目
  3. 回顾昨日进度（读取 DailyRecord/昨日.md）
  4. 推荐1道适合当前水平的题目（Easy 优先）
  5. ⭐ 在 Mythouth/YYYY-MM-DD.md 中追加题目模板（见"模板生成机制"）
  6. 展示题目并进入Step 1（问题分析）
  7. 提出2-3个引导性问题
  8. ⏸️ 等待用户回答（不催促）
```
